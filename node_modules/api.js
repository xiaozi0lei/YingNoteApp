...
// private key
var optionsun = {
    cert: fs.readFileSync(__dirname + '/../bak/214059172280191.pem'),
	// 替换证书格式
    // ca: fs.readFileSync(__dirname + '/../bak/server.crt'),
    timeout: 10000
};

var crtKey = fs.readFileSync(__dirname + '/../bak/214059172280191.pem');
// 替换证书格式
// var crtKey = fs.readFileSync(__dirname + '/../bak/server.crt');
...
var Api = {
	...
	logout: function (callback) {
		needle.post(this.getUrl('auth/logout'), optionsun, function(error, response) {
			callback();
		});
	},
	...
	getSyncNotebooks: function(afterUsn, maxEntry, callback) {
		...
		// console.log(url);
		needle.get(url, optionsun, function(error, response) {
			me.checkError(error, response);
			if(error) {
				console.log(error);
				return callback && callback(false);
	...
	getImage: function(fileId, callback) {
		...
		needle.get(url, optionsun, function(err, resp) {
	...
	addNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			console.log(data);
			needle.post(me.getUrl('notebook/addNotebook'), data, optionsun, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
	...
	updateNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			log('update notebook');
			log(data);
			needle.post(me.getUrl('notebook/updateNotebook'), data, optionsun, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
					log('err');
					log(err);
	...
	deleteNotebook: function(notebook, callback) {
		...
		log('delete notebook');
		needle.post(me.getUrl('notebook/deleteNotebook'), data, optionsun, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	getNote: function(noteId, callback) {
		var me = this;
		needle.get(me.getUrl('note/getNote', {noteId: noteId}), optionsun, function(error, response) {
			me.checkError(error, response);
			if(error) {
				return callback && callback(false);
			}
		...
		Notebook.getServerNotebookIdByNotebookId(note.NotebookId, function(serverNotebookId) {
			...
			try {
				needle.post(me.getUrl('note/addNote'), data, 
					{cert: crtKey, multipart: needMultiple},
					function(err, resp) {
						me.checkError(err, resp);
						if(err) {
							return callback(err);
	...
	updateNote: function(note, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(note.NotebookId, function(serverNotebookId) {
			...
			needle.post(me.getUrl('note/updateNote'), data, {cert: crtKey, multipart: needMultiple}, function(err, resp) {
	...
	deleteTrash: function(note, callback) {
		...
		// 这里要重新require下, 不然为{}
		Note = require('note');
		needle.post(me.getUrl('note/deleteTrash'), data, optionsun, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	exportPdf: function(noteId, callback) {
		...
		needle.get(me.getUrl('note/exportPdf', {noteId: noteId}), optionsun, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	addTag: function(title, callback) {
		var me = this;
		needle.post(me.getUrl('tag/addTag'), {tag: title}, optionsun, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	deleteTag: function(tag, callback) {
		var me = this;
		needle.post(me.getUrl('tag/deleteTag'), {tag: tag.Tag, usn: tag.Usn}, optionsun, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	post: function() {
		var me = this;
		var options = {cert: crtKey, headers: { 'X-Custom-Header': 'Bumbaway atuna' }};
	...
	getImageTest: function(callback) {
		needle.get('http://localhost:9000/images/logo.png', optionsun, function(err, resp) {
			// log(resp.body);
			/*
			{ 'accept-ranges': 'bytes',
			  'content-disposition': 'inline; filename="logo.png"',
	...
	uploadImage: function() {
		...
		needle.post('http://localhost/phpinfo.php', data, { cert: crtKey, multipart: true }, function(err, resp, body) {
