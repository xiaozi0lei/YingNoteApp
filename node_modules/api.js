...
var Api = {
	...
	auth: function(email, pwd, host, callback) { 
        ...
        var optionsun = {
            ca: fs.readFileSync('bak/server.crt'),
            timeout: 10000
        };
        needle.post(this.getUrl('auth/login'), {email: email, pwd: pwd}, optionsun, function(error, response) {
	...
	logout: function (callback) {
		needle.post(this.getUrl('auth/logout'), {ca: fs.readFileSync('bak/server.crt'), timeout: 10000}, function(error, response) {
			callback();
		});
	},
	...
	getSyncNotebooks: function(afterUsn, maxEntry, callback) {
		...
		// console.log(url);
		needle.get(url, { ca: fs.readFileSync('bak/server.crt') },
	...
	getSyncNotes: function(afterUsn, maxEntry, callback) {
		...
		log(url);
		needle.get(url, { ca: fs.readFileSync('bak/server.crt') }, function(error, response) {
			me.checkError(error, response);
			if(error) {
	...
	getLastSyncState: function(callback) {
		...
		// console.log(url);
		needle.get(url, {ca: fs.readFileSync('bak/server.crt'), timeout: 10000}, function(error, response) {
	...
	getImage: function(fileId, callback) {
		...
		needle.get(url, {ca: fs.readFileSync('bak/server.crt')}, function(err, resp) {
	...
	addNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			console.log(data);
			needle.post(me.getUrl('notebook/addNotebook'), data, {ca: fs.readFileSync('bak/server.crt')}, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
	...
	updateNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			log('update notebook');
			log(data);
			needle.post(me.getUrl('notebook/updateNotebook'), data, {ca: fs.readFileSync('bak/server.crt')}, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
					log('err');
					log(err);
	...
	deleteNotebook: function(notebook, callback) {
		...
		log('delete notebook');
		needle.post(me.getUrl('notebook/deleteNotebook'), data, {ca: fs.readFileSync('bak/server.crt'), timeout: 10000}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	getNote: function(noteId, callback) {
		var me = this;
		needle.get(me.getUrl('note/getNote', {ca: fs.readFileSync('bak/server.crt'), noteId: noteId}), function(error, response) {
			me.checkError(error, response);
			if(error) {
				return callback && callback(false);
			}
	...
	updateNote: function(note, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(note.NotebookId, function(serverNotebookId) {
			...
			needle.post(me.getUrl('note/updateNote'), data, {ca: fs.readFileSync('bak/server.crt'), multipart: needMultiple}, function(err, resp) {
	...
	deleteTrash: function(note, callback) {
		...
		// 这里要重新require下, 不然为{}
		Note = require('note');
		needle.post(me.getUrl('note/deleteTrash'), data, {ca: fs.readFileSync('bak/server.crt'), timeout: 10000}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	exportPdf: function(noteId, callback) {
		...
		needle.get(me.getUrl('note/exportPdf', {ca: fs.readFileSync('bak/server.crt'), noteId: noteId}), function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	addTag: function(title, callback) {
		var me = this;
		needle.post(me.getUrl('tag/addTag'), {tag: title}, {ca: fs.readFileSync('bak/server.crt')}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	deleteTag: function(tag, callback) {
		var me = this;
		needle.post(me.getUrl('tag/deleteTag'), {tag: tag.Tag, usn: tag.Usn}, {ca: fs.readFileSync('bak/server.crt'), timeout: 10000}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	getImageTest: function(callback) {
		needle.get('http://localhost:9000/images/logo.png', {ca: fs.readFileSync('bak/server.crt')}, function(err, resp) {
			// log(resp.body);
			/*
			{ 'accept-ranges': 'bytes',
			  'content-disposition': 'inline; filename="logo.png"',
	...
	uploadImage: function() {
		...
		needle.post('http://localhost/phpinfo.php', data, { ca: fs.readFileSync('bak/server.crt'), multipart: true }, function(err, resp, body) {
